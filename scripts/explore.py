...
class jackal_explore:
    def __init__(self):
        ...
        self.feedback: MoveBaseActionFeedback = MoveBaseActionFeedback()
        self.prevpnt = -1
        self.nextpnt = 0
        self.sample = []
        self. stuckpnt = []
        ...
        self.map_sub = rospy.Subscriber(
            '/projected_map', OccupancyGrid, self.mapCb, queue_size=1)
        self.global_sub = rospy.Subscriber(
            '/move_base/global_costmap/costmap', OccupancyGrid, self.costCb, queue_size=1)
        self.globalup_sub = rospy.Subscriber(
            '/move_base/global_costmap/costmap_updates', OccupancyGridUpdate, self.costupCb, queue_size=1)
        self.result_sub = rospy.Subscriber(
            'move_base/result', MoveBaseActionResult, self.sendNavCb)
        self.feedback_sub = rospy.Subscriber(
            'move_base/feedback', MoveBaseActionFeedback, self.feedbackCb)
        self.poly_sub = rospy.Subscriber(
            '/clicked_point', PointStamped, self.polyCb, queue_size=1)
    ...
    def costupCb(self, data: OccupancyGridUpdate):
        self.flagg = 1
        cost_update = data
    ...
    def costCb(self, data: OccupancyGrid):
        self.newcost = []
        self.flagm = 1
        self.costmap = data
    ...
    def mapCb(self, data: OccupancyGrid):
        self.grid = data
    ...
    def feedbackCb(self, data: MoveBaseActionFeedback):
        offsetx = [1, -1, 1, -1]
        offsety = [1, -1, -1, 1]
    ...
    def sendNavCb(self, data):
        ...
        if result.status.text == "Goal reached.":
            if self.flagg == 1 and self.init == 1:
                ...
                mc = interp1d([ox, ox+width*res],
                              [0, width], bounds_error=False)
                mr = interp1d([oy, oy+height*res],
                              [0, height], bounds_error=False)
                ...
                mw = interp1d([0, width], [ox, ox+width*res],
                              bounds_error=False)
                mh = interp1d([0, height], [oy, oy+height*res],
                              bounds_error=False)
                    ...
                    if self.pntlist:
                        for i in range(len(self.pntlist)):
                            if calc_distance(pnt1, self.pntlist[i]) < 5:
                                ...
                                if len(frontier_pnts) > 0:
                                    idx = find_closest(
                                        [int(c), int(r)], frontier_pnts)
                                    nextpnt = frontier_pnts[idx]
                                    pnt1 = [mw(nextpnt[0]), mh(nextpnt[1])]
                                    rospy.loginfo(
                                        'sending number '+str(i+2) + ' in list')
                                else:
...
# plt.imshow(frontier_mat, cmap='gray', origin='lower')
    ...
    frontmat = cv2.dilate(frontmat, kernel, iterations=1)
    contour = cv2.findContours(
        frontmat.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[-2]
    
# plt.subplot(223)
# plt.imshow(np.array(mat).astype(np.uint8),cmap='gray',origin='lower')
    ...
    if len(contour) > 0:
        ...
        for i in range(len(contour)):
            ...
            if radius > 25:
                ...
                res = cv2.circle(
                    res, (int(center[0]), int(center[1])), int(radius), 75, 2)
        ...
        #     frontier_pnts = frontier_filter
        # res = cv2.drawContours(frontmat, contour, -1, (0,255,0), 3)
# plt.subplot(224)
